function NIFTI_NORDIC_nipype(fn_magn_in,fn_phase_in,fn_out,ARG_path)
    ...
    if ~isfield(ARG,'factor_error')
        ARG.factor_error=1.0;  % error in gfactor estimation. >1 use higher noisefloor <1 use lower noisefloor
    end
    ...
    if ARG.magnitude_only~=1
        ...
        phase_range=single(max(phase_data(:)));
        phase_range_min=single(min(phase_data(:)));
        if ~exist('info_phase')
            info_phase.Datatype=class(phase_data);
            info.Datatype=class(mag_data);
        end
        % Here, we combine magnitude and phase data into complex form
        fprintf('Phase should be -pi to pi...\n')

        % TS: Convert phase to single and then scale the phase from -pi to pi
        ...
        phase_data = (single(phase_data)./range_norm -range_center)*2*pi;

        % TS: Combine magnitude and phase into complex-valued data
        complex_data=single(mag_data)  .* exp(1i*phase_data);


        if 0
            if strmatch(info_phase.Datatype,'uint16')
                phase_data = single(phase_data)/phase_range*2*pi;
                complex_data=single(mag_data)  .* exp(1i*phase_data);
            elseif strmatch(info_phase.Datatype,'int16')
                phase_data = (single(phase_data)+1-(phase_range+1)/2)/(phase_range+1)*2*pi;
                complex_data=single(mag_data)  .* exp(1i*phase_data);
            elseif strmatch(info_phase.Datatype,'single')
                phase_range_min=min(phase_data(:));
                range_norm=phase_range-phase_range_min;
                range_center=(phase_range+phase_range_min)/range_norm*1/2;
                phase_data = (single(phase_data)./range_norm -range_center)*2*pi;
                complex_data=single(mag_data)  .* exp(1i*phase_data);
    ...
    % TS: Select the first volume of the complex data and take the absolute values
    TEMPVOL=abs(complex_data(:,:,:,1));
    % TS: Find the minimum non-zero value in the first volume and divide the complex data by it
    ...
    % TS: Create a copy of the complex data for processing (what processing?)
    KSP2 = complex_data;
    matdim = size(KSP2);

    % TS: Reshape the absolute values of the complex data to a 2D array of shape (X*Y*Z, T)
    % TS: Then take the mean over the first dimension to get mean array of shape (1, T)
    tt = mean(reshape(abs(KSP2),[], size(KSP2, 4)));
    % TS: Find the index of the volumes that are greater than 95% of the maximum mean value
    [idx]=find(tt>0.95*max(tt));
    % TS: Create mean 3D array from high-mean volumes of shape (X, Y, Z)
    ...
    if 1
        ...
        % TS: Create mean 3D array from all non-noise volumes of shape (X, Y, Z)
        ...
        % TS: Multiply the mean array by either 1 or 0 (default is 0)
        meanphase=meanphase*ARG.phase_slice_average_for_kspace_centering;
    end



    % TS: IGNORE THIS CHUNK! IT'S UNUSED!
    % TS: Loop over slices backwards
    for slice=matdim(3):-1:1
        % TS: Loop over volumes forward, including the noise volumes
        for n=1:size(KSP2,4); % include the noise
            % TS: Create array that never gets used. WHY?????
            KKSP2(:,:,slice,n)=KSP2(:,:,slice,n).*exp(-i*angle(meanphase(:,:,slice)));
        end
    end

    % TS: Preallocate 4D array of zeros
    DD_phase=0*KSP2;

    % TS: If the temporal phase is 1 - 3, do a standard low-pass filtered map
    ...
    if ARG.temporal_phase>0; % Standard low-pass filtered map
        ...
        for slice=matdim(3):-1:1
            % TS: Loop over volumes forward, including the noise volumes
            for n=1:size(KSP2,4);
                % TS: Grab the 2D slice of the 4D array
                tmp=KSP2(:,:,slice,n);
                % TS: Apply 1D FFT to the 2D slice
                for ndim=[1:2]
                    tmp=ifftshift(ifft(ifftshift( test ,ndim),[],ndim),ndim+0);
                end
                % TS: Apply Tukey window to the filtered 2D slice
                ...
                % TS: Apply 1D IFFT to the filtered 2D slice and store in the 4D array
                for ndim=[1:2]
    ...
    % TS: If the temporal phase is 2, do a secondary step for filtered phase with residual spikes
    % This is not enabled by default, for fMRI *or* DWI
    ...
    for slice=matdim(3):-1:1
        % TS: Loop over volumes forward, including the noise volumes
        for n=1:size(KSP2,4);
            % TS: Multiply the 4D array by the exponential of the angle of the filtered phase
    ...
    % TS: Pretty sure this is never used
    if isfield(ARG,'use_magn_for_gfactor')
        ...
        if (isempty(ARG.kernel_size_gfactor) | size(ARG.kernel_size_gfactor,2)<3)
            % TS: Select first 90 (or fewer, if run is shorter) volumes from 4D array
            KSP2=(KSP2(:,:,1:end,1:min(90,end),1));  % should be at least 30 volumes
        else
            % TS: Select first N volumes from 4D array, based on ARG.kernel_size_gfactor(4)
    ...
    % TS: Replace NaNs and Infs with zeros
    KSP2(isnan(KSP2))=0;
    KSP2(isinf(KSP2))=0;
    master_fast=1;
    ...
    % TS: Preallocate 3D arrays of zeros
    KSP_weight=KSP2(:,:,:,1)*0;
    NOISE=KSP_weight;
